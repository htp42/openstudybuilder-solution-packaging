"""
Tests for /studies/{uid}/study-activity-groups endpoints
"""
...
second_general_activity_group: ActivityGroup
...
def test_data():
    ...
    second_general_activity_group = TestUtils.create_activity_group(
        name="Second General"
    )
...
def test_post_and_get_all_study_activity_groups(api_client):
    ...
    for i in range(20):
        ...
        study_activity_group_into_study_study_activity_subgroup_mapping.setdefault(
            study_activity_group_uid, []
        ).append(study_activity_subgroup_uid)
    ...
    for study_activity_group in study_activity_groups:
        ...
        # Check if StudyActivityGroup returns correct StudyActivitySubGroups
        assert sorted(study_activity_group["study_activity_subgroup_uids"]) == sorted(
            study_activity_group_into_study_study_activity_subgroup_mapping[
                study_activity_group["study_activity_group_uid"]
            ]
        )
...
def test_study_activity_group_reordering(api_client):
    ...
    weight_sa1 = TestUtils.create_study_activity(
        ...
        activity_group_uid=second_general_activity_group.uid,
    ...
    assert study_activities[3]["study_activity_group"]["order"] == 2
    ...
    # Reorder first SA
    response = api_client.patch(
        f"/studies/{test_study.uid}/study-activity-groups/{weight_sa2.study_activity_group.study_activity_group_uid}/order",
        json={
            "new_order": 3,
        },
    )
    ...
    assert (
        res["message"]
        == "The maximum new order is (2) as there are 2 StudyActivityGroups in EFFICACY soa group and order (3) was requested"
    )
    ...
    assert (
        res["message"]
        == f"The order (1) for study activity group {weight_sa2.study_activity_group.activity_group_name} was not changed"
    )
    ...
    res = response.json()
    assert (
        res["study_activity_group_uid"]
        == weight_sa2.study_activity_group.study_activity_group_uid
    )
    assert res["order"] == 2
    ...
    assert study_activities[1]["study_soa_group"]["order"] == 1
    assert study_activities[1]["study_activity_group"]["order"] == 2
    assert study_activities[1]["study_activity_subgroup"]["order"] == 1
    assert study_activities[1]["order"] == 1
    assert study_activities[2]["activity"]["uid"] == body_measurement_sa.activity.uid
    assert study_activities[2]["study_soa_group"]["order"] == 1
    assert study_activities[2]["study_activity_group"]["order"] == 2
    assert study_activities[2]["study_activity_subgroup"]["order"] == 1
    ...
    # Get all SA after SoA Group patch to check if groups are reordered
    response = api_client.get(f"/studies/{test_study.uid}/study-activities")
    assert_response_status_code(response, 200)
    study_activities = response.json()["items"]
    assert len(study_activities) == 4
