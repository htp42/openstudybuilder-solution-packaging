    ...
    for criterion in criteria:
        if criterion.status == "Final":
            study_criteria_selection_service.make_selection(
                ...
                selection_create_input=StudySelectionCriteriaInput(
                    criteria_uid=criterion.uid
                ),
            )
...
def test_get_criterion(api_client):
    ...
    assert res["template"]["guidance_text"] == criteria_template.guidance_text
    assert res["template"]["sequence_id"] == "CI1"
...
def test_get_criteria_pagination(api_client):
    ...
    assert len(
        [criteria for criteria in criteria if criteria.status == "Final"]
...
def test_get_criteria(
    ...
    assert res["total"] == (
        len([criteria for criteria in criteria if criteria.status == "Final"])
        if total_count
        else 0
    )
    ...
    for item in res["items"]:
        assert set(list(item.keys())) == set(CRITERIA_FIELDS_ALL)
        for key in CRITERIA_FIELDS_NOT_NULL:
...
def test_get_all_parameters_of_criteria(api_client):
...
def test_get_versions_of_criteria(api_client):
    ...
    assert res[1]["uid"] == criteria[1].uid
...
def test_headers(api_client, field_name):
    ...
    for criterion in criteria:
        ...
        if value and criterion.status == "Final":
...
def test_get_studies_of_criteria(api_client):
...
def test_create_criteria(api_client):
    text_value = TestUtils.create_text_value()
    data = {
        "criteria_template_uid": criteria_template.uid,
        "library_name": "Sponsor",
    ...
    assert set(res.keys()) == set(CRITERIA_FIELDS_ALL)
    for key in CRITERIA_FIELDS_NOT_NULL:
        assert res[key] is not None
...
def test_update_criteria(api_client):
    ...
    response = api_client.patch(f"{URL}/{criteria[2].uid}", json=data)
    res = response.json()
...
def test_delete_criteria(api_client):
...
def test_inactivate_criteria(api_client):
...
def test_reactivate_criteria(api_client):
...
def test_preview_criteria(api_client):
    text_value = TestUtils.create_text_value()
    data = {
        "criteria_template_uid": criteria_template.uid,
        "library_name": "Sponsor",
...
def test_criteria_audit_trail(api_client):
...
def test_change_parameter_numbers_of_criteria_after_approval(
    api_client,
):
    data = {
    ...
    response = api_client.patch(f"{URL}/{criteria[4].uid}", json=data)
    res = response.json()
...
def test_cannot_update_criteria_without_change_description(api_client):
    data = {"name": "Default name with [TextValue]", "parameter_terms": []}
    response = api_client.patch(f"{URL}/{criteria[1].uid}", json=data)
    res = response.json()
...
def test_cannot_update_criteria_in_final_status(api_client):
    data = {
        "parameter_terms": [
            {
                "position": 1,
