          ...
          <xsl:choose>
            <xsl:when test="MeasurementUnitRef">
                        ...
                        <xsl:for-each select="CodeListRef">
                          <div class="col-sm-6 {$labelColor} border text-left">
                            <xsl:for-each select="//CodeList[@OID = current()/@CodeListOID]/CodeListItem">
                              ...
                              <input type="{$displayType}" class="{$labelColor}" id="{@CodedValue}" name="{../@OID}" value="{@CodedValue}" /> &#160;<xsl:apply-templates select="Decode" />&#160;[<xsl:value-of select="@CodedValue" />]&#160;<br />
                              <span class="oidinfo oid collapse">[OID=<xsl:value-of select="@osb:OID" />, Version=<xsl:value-of select="@osb:version" />
                            </span>
                          </xsl:for-each>
                          ...
                          <xsl:when test="./@Origin = 'Protocol Value' or ./@Origin = 'Derived Value'">
                            <xsl:choose>
                              <xsl:when test="./Alias/@Context = 'DEFAULT_VALUE'">
                                ...
                                <xsl:otherwise>
                                  <input type="{@DataType}" class="{$labelColor}" id="item{@OID}"
                                    name="{@Name}" min="4" maxlength="{@Length}" 
                      ...
                      <xsl:choose>
                        <xsl:when test="./@osb:instruction != 'None'">
                          <div class="alert alert-secondary text-left help collapse" role="alert">
                            ...
                            <xsl:value-of disable-output-escaping="yes" select="@osb:instruction" />
                          </div>
                        </xsl:when>
                      </xsl:choose>
                      <xsl:choose>
              ...
              <xsl:template match="Question">
                ...
                <xsl:value-of select="TranslatedText" />&#160; <xsl:choose>
