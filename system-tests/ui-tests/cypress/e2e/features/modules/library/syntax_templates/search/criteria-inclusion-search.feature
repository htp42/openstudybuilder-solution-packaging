...
Feature: Library - Syntax Templates - Inclusion Criteria - Search
    ...
    Scenario: [Test data] Data for search tests is created
        Given [API] Search Test - Create first 'Inclusion' criteria template
        And [API] Search Test - Create second 'Inclusion' criteria template

    @smoke_test
    Scenario: [Table][Search][Postive case] User must be able to search created Inclusion Criteria template
    ...
    Scenario: [Table][Search][Negative case] User must be able to search not existing Inclusion Criteria and table will correctly filtered
        Given The 'library/criteria_templates/Inclusion/parent' page is opened
