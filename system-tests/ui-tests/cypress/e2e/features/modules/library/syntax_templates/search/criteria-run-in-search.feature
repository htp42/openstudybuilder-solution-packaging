...
Feature: Library - Syntax Templates - Run-in Criteria - Search
    ...
    Scenario: [Test data] Data for search tests is created
        Given [API] Search Test - Create first 'Run-in' criteria template
        And [API] Search Test - Create second 'Run-in' criteria template

    @smoke_test
    Scenario: [Table][Search][Postive case] User must be able to search created Run-in Criteria template
    ...
    Scenario: [Table][Search][Negative case] User must be able to search not existing Run-in Criteria and table will correctly filtered
        Given The 'library/criteria_templates/Run-in/parent' page is opened
